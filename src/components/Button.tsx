...
export default function Button({
    ...
    return(
        <PrismicNextLink
            ...
            className)}
            >
            <span className="absolute inset-0 z-0 h-full translate-y-9 bg-amber-300 transition-transform duration-300 ease-in-out group-hover:translate-y-0"></span>
            <span className="relative flex items-center justify-center gap-2">
                {label} {showIcon && <HiArrowNarrowRight className="inline-block"/>}
            </span>
        </PrismicNextLink>
    )
}